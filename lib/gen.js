...
function gen_level(out, level) {
  ...
  out.line(`_${level.name}.prototype.get_main = _${level.name}.prototype._compute_${level.main}`)

  out.line()

  out.line(`_${level.name}.prototype.get_eval = function(forName)`)
  out.block(() => {
    out.line(`var uses = this['_uses_' + forName]`)
    out.line('var self = this')
    out.line('return function evaluate(name)')
    out.block(() => {
      out.line('let actualName = uses[name]')
      out.line('if (!actualName) throw new Error(`\'${name}\' is not declared as used from \'${forName}\'`)')
      out.line(`var getter = self['_get_' + actualName]`)
      out.line('var future = new rt.Future')
      out.line('rt.poll(future, self, getter)')
...
function gen_function(out, level, name, def) {
  ...
  out.block(() => {
    ...
    def.forEachPreAndArgument(dep => {
      let rhs = dep == 'evaluate' ? `this.get_eval('${name}')` : `this._get_${dep}()`
