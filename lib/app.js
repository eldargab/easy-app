...
function checkName(name) {
  ...
  if (name[0] == '_') throw new Error(`Bad name '${name}'. Names can't begin with underscore`)
}
...
App.prototype.install = function(ns, app) {
  ...
  function rename_dep(name) {
    return app.defs[name] ? add_namespace(ns, name) : name
  }
...
App.prototype.compile = function(main) {
  ...
  let js = generate(app)
  let fn = new Function('poll', 'Future', 'toError', 'defs', js)
  return fn(poll, go.Future, go.toError, app.defs)
}
