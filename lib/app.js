...
App.prototype.undefine = function(task) {
  ...
  this['eval_' + task] = undefined
  this['_eval_' + task] = undefined
}
...
App.prototype.eval = function(task, cb) {
  cb = cb || noop
  var fn = this['eval_' + task]
  if (!fn) return cb(new Error('Task ' + task + ' is not defined'))
  fn.call(this, cb)
}

App.prototype.generateFunctions = function(t) {
  var code = ''
  code += eval_$TASK + '\n'
  code += _eval_$TASK + '\n'
  code += 'app.eval_$TASK = eval_$TASK\n'
  code += 'app._eval_$TASK = _eval_$TASK\n'

  code = code.replace(/\$TASK/g, t.name)

  var fn = new Function(
    't',
    'app',
    'Promise',
    'printDoubleCallbackWarning',
    'evalWithDeps',
    'find',
    code
  )

  fn(t, this, Promise, printDoubleCallbackWarning, evalWithDeps, find)

  t._deps = t.deps.map(function(name) {
    if (name == 'done') return function(app, ondone, cb) {
      cb(null, ondone)
    }

    if (name == 'eval') return function(app, ondone, cb) {
      cb(null, createEval(t.namespace, app))
    }

    var code = 'app.eval_' + name + '(cb)'
    return new Function('app', 'ondone', 'cb', code)
  })
}

function eval_$TASK(cb) {
  var val = this.values.$TASK
  if (val !== undefined) {
    val instanceof Error ? cb(val) : cb(null, val)
    return
  }
...
function _eval_$TASK(cb) {
  var app = t.layer ? find(this, t.layer) : this
    , done = false
    , promise
  ...
  function ondone(err, val) {
    ...
    if (app.promises.__owner === app) {
      app.promises.$TASK = null // cleanup
    }
  ...
  if (!done) {
    app.thisPromises().$TASK = promise = new Promise
  }
}
...
function evalWithDeps(app, t, deps, start, ondone) {
  var sync = true
  for (var i = start; i < t._deps.length; i++) {
    var fn = t._deps[i]
    var done = false
    fn(app, ondone, function(err, val) {
