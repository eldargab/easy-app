...
App.prototype._def = function(t) {
  ...
  t._deps = t.deps.map(function(name, index) {
    return depFn(name, index, t)
  })
}
...
App.prototype.eval = function(task, cb) {
  ...
  if (val === undefined) return this._eval(task, cb)
  val instanceof Error ? cb(val) : cb(null, val)
}

App.prototype._eval = function(task, cb) {
...
function depFn(name, i, t) {
  if (name == 'done') return function(app, deps, ondone) {
    deps[i] = ondone
    return true
  }

  if (name == 'eval') return function(app, deps, ondone) {
    deps[i] = createEval(t.namespace, app)
    return true
  }

  return function(app, deps, ondone) {
    var val = app.values[name]
    if (val !== undefined) {
    ...
    app._eval(name, function(err, val) {
      if (err) return ondone(err, '__DEP__')
      deps[i] = val
      done = true
      if (async) evalWithDeps(app, t, deps, i + 1, ondone)
    })
